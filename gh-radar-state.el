...
(defun gh-radar-state-load-cache ()
  ...
  (when-let* ((cached (gh-radar-cache-load)))
    ...
    (let ((host (gh-radar-cache-get-setting :github-host :unspecified)))
      (unless (eq host :unspecified)
        (setq gh-radar-github-host host)))
    (let ((lim (gh-radar-cache-get-setting :recent-items-limit :unspecified)))
      (unless (eq lim :unspecified)
        (setq gh-radar-recent-items-limit lim)))
